...
gggtable <- function(data) {
  ...
  if (tag_pos == "manual") {
    ...
    if (tag_pos == "topleft") {
      ...
      plot_table$widths <- grid::unit.c(
        plot_table$widths[-ncol(plot_table)],
        tag_width
      ) # ******
      plot_table$heights <- grid::unit.c(tag_height, plot_table$heights[-1]) # ******
      plot_table <- gtable::gtable_add_grob(
      ...
      plot_table$widths <- grid::unit.c(tag_width, plot_table$widths[-1]) # ******
      plot_table$heights <- grid::unit.c(
        plot_table$heights[-nrow(plot_table)],
        tag_height
      ) # ******
      plot_table <- gtable::gtable_add_grob(
        plot_table,
  ...
  # Margins
  plot_table <- gtable::gtable_add_rows(
    plot_table,
    theme$plot.margin[1],
    pos = 0
  ) # ******
  ...
  plot_table <- gtable::gtable_add_cols(
    plot_table,
    theme$plot.margin[4],
    pos = 0
  ) # ******
