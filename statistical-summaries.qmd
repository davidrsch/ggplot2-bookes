...
La elección de una variable de ponderación afecta profundamente lo que estamos viendo en la trama y las conclusiones que sacaremos. Hay dos atributos estéticos que se pueden utilizar para ajustar los pesos. En primer lugar, para geoms simples como líneas y puntos, use la estética de tamaño:
...
-   Agrupe los puntos y cuente el número en cada contenedor, luego visualice ese recuento (la segunda generalización del histograma), `geom_bin2d()`. Dividir la trama en muchos cuadrados pequeños puede producir artefactos visuales que distraigan. [@carr:1987] sugiere usar hexágonos en su lugar, y esto se implementa en `geom_hex()`, usando el paquete **hexbin** [@hexbin]. \index{hexbin}
